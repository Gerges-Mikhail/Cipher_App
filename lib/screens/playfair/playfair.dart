...
class playfairFormText extends StatefulWidget {
  const playfairFormText({Key? key}) : super(key: key);
  @override
  State<playfairFormText> createState() => _playfairFormTextState();
}

class _playfairFormTextState extends State<playfairFormText> {
  ...
  void playfairEncrypt(String text, String key) {
    ...
    //   }
    // }
    for (var i = 0 , j =1 ; i < text.length - 1; i+=2 , j+=2) {
      if (text[i] == text[j]) {
        text =
        '${text.substring(0, i + 1)}x${text.substring(i + 1, text.length)}';
      }
    }
    //check text if even or ood
    if (text.length % 2 != 0) text += 'x';

    var matrix = List.generate(5, (i) => List.filled(5, '', growable: false)),
        index = 0;

    //check duplicate ==table doesnot contain the same char twice
  ...
  void playfairDecrypt(String text, String key) {
    ...
    text = text.toLowerCase().replaceAll(' ', '').replaceAll('j', 'i');
    key = key.toLowerCase().replaceAll(' ', '').replaceAll('j', 'i');

    if (text.length % 2 != 0) {
      showAlert("Can't decrypt because the text is not valid(odd length)");
    }
    else{
      if(text.contains( RegExp(r'(\w)1\{1}')))
      {
        showAlert("Can't decrypt because the text is not valid(duplicate)");
      }
      else
      {
        var matrix = List.generate(5, (i) => List.filled(5, '', growable: false)),
